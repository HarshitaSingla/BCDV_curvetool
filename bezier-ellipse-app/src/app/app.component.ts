...
//working perfect with angular 19
...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    // ✅ No longer needed since scripts are loaded via angular.json
  }

  // ✅ Proper setup of Leaflet.Editable
  onMapReady(mapInstance: L.Map) {
    this.map = mapInstance;
    (this.map as any).options.editable = true;
    ...
    if (!(this.map as any).editTools) {
      console.error('❌ Leaflet.Editable not initialized. Check script load order in angular.json.');
    } else {
      console.log('✅ editTools is available:', (this.map as any).editTools);
    }
  }
